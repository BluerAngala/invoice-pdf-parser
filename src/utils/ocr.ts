...
export async function recognizeInvoice(imageUrl: string, fileName: string, pdfText?: string): Promise<InvoiceData> {
  // 如果有PDF文本，优先从文本提取
  if (pdfText) {
  ...
  try {
    const response = await fetch(import.meta.env.VITE_SILICONFLOW_API_URL || 'https://api.siliconflow.cn/v1/chat/completions', {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          ...
          content: [
            ...
            { type: 'text', text: '识别发票，返回JSON: {invoiceNumber,invoiceCode,date,seller,buyer,amount,taxAmount,totalAmount}' }
          ]
        }],
        temperature: 0.1,
        max_tokens: 1000
...
function parseInvoiceText(text: string): InvoiceData {
  // 发票号码
  const invoiceNumber = (text.match(/发票号码[：:\s]*(\d{8,20})/) || 
                        text.match(/号码[：:\s]*(\d{8,20})/))?.[1] || ''
  ...
  if (namesMatch) {
    ...
    seller = text.match(/销\s*售\s*方[\s\S]{0,100}?名\s*称[：:\s]*([^\s\n统一社会]{2,50})/)?.[1]?.trim() || ''
    buyer = text.match(/购\s*买\s*方[\s\S]{0,100}?名\s*称[：:\s]*([^\s\n统一社会]{2,50})/)?.[1]?.trim() || ''
  }
